...
Current status of the 5 réalisations (all sourced from `Source/`, which is gitignored — see below):
1. `realisation-1.html` — Intégration API GLS. Done.
2. `realisation-2.html` — Reprise API Géodis. Done.
3. `realisation-3.html` — Évolution de Gedim. Done.
4. `realisation-4.html` — Automatisation des Garanties d'Origine (EDF), sourced from `Source/Rapport d'activité Cellarius Matteo.pdf` and `Source/Journal_Note Alternance EDF.pdf` (previous, non-Romus apprenticeship). This **replaced** an earlier Swagger API page (`Source/Journal_note Alternance ISCOD.pdf` originally listed Swagger as the 4th candidate, alongside Gedim, Site IA RomusWorld, API GLS, Reprise API Géodis) — there wasn't enough journal material to write a full Swagger page, so the user chose to feature an EDF project instead. Done.
5. `realisation-5.html` — Automatisation des prévisions de consommation (EDF), the "Macro Spot Horaire" from the same EDF rapport/journal as Réalisation 4 (two-part Outlook+Excel VBA macro consolidating client consumption forecasts). Chosen over the earlier "Site IA RomusWorld" candidate. Done.
...
- **Réalisation naming.** The rubric requires réalisation names to be "évocateurs et indépendants du contexte de l'école ou de l'entreprise d'accueil." The current 5 titles ("API GLS", "API Géodis", "Gedim", the two "(EDF)" pages) are named directly after the client/carrier/employer, which cuts against this rule. Don't rename unilaterally — this affects file names, navbar links, and every cross-link; confirm the new names with the user first.
